...
// ─── Local SQLite Memory ──────────────────────────────────────────────────────
// Pure local session memory — no 0G, no decentralized storage
// Stores: research sessions, user profiles, general KV memory
...
export class LocalMemory {
  ...
  upsertUser(profile: UserProfile): void {
    this.db.prepare(`
      INSERT INTO users (id, company, industry, competitors, research_goals, created_at, updated_at)
      VALUES (@id, @company, @industry, @competitors, @researchGoals, @createdAt, @updatedAt)
      ON CONFLICT(id) DO UPDATE SET
        company = @company, industry = @industry,
        competitors = @competitors, research_goals = @researchGoals,
  ...
  set(key: string, value: unknown): void {
    this.db.prepare(`
      INSERT INTO memory (key, value, stored_at)
      VALUES (@key, @value, @storedAt)
      ON CONFLICT(key) DO UPDATE SET value = @value, stored_at = @storedAt
    `).run({ key, value: JSON.stringify(value), storedAt: new Date().toISOString() });
  }
