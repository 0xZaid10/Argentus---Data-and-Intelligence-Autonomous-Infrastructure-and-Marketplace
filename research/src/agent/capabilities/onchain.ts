...
import { getProtocolTVLs, getChainTopProtocols, formatTVL } from '../../services/defillama.js';
import {
  buildWalletProfile,
  getTokenProfile,
  traceFundFlow,
...
// Direct RPC calls — no privacy routing layer

export interface OnchainParams {
  type: 'wallet' | 'token' | 'fundflow';
  target: string;
  chain?: string;
  chains?: string[];
  maxHops?: number;
...
async function analyzeWallet(params: OnchainParams): Promise<CapabilityResult> {
  ...
  let tvlContext = ''
  try {
    const chainName = chain === 'ethereum' ? 'Ethereum' : chain === 'solana' ? 'Solana' : chain
    const topProtocols = await getChainTopProtocols(chainName, 10)
    if (topProtocols.length > 0) {
      tvlContext = '\nDeFiLlama TVL Data (live):\n' + topProtocols.map(p =>
        `${p.name}: ${formatTVL(p.tvl)} (${p.tvl_7d_change > 0 ? '+' : ''}${p.tvl_7d_change?.toFixed(1)}% 7d)`
      ).join('\n')
    }
  } catch {}

  const analysis = await llm.prompt(
    `Analyze this blockchain wallet profile and provide intelligence.
...
- Wallet type: ${profile.walletType}
...
TOP RECENT TRANSACTIONS (last 10):
...
6. Risk assessment
7. Key insights`,
    'You are an onchain intelligence analyst. Be specific and data-driven. Flag suspicious patterns.',
    { temperature: 0.3 }
  );
...
async function analyzeToken(params: OnchainParams): Promise<CapabilityResult> {
  ...
  const analysis = await llm.prompt(
    `Analyze this token's onchain data and provide intelligence.
...
async function traceFunds(params: OnchainParams): Promise<CapabilityResult> {
  ...
  const llm = getLLM();
  const minValueETH = minValueUSD / 3000;
  ...
  const analysis = await llm.prompt(
    `Analyze this fund flow trace and identify patterns.
...
export async function runOnchainResearch(params: OnchainParams): Promise<CapabilityResult> {
  logger.info('[Capability:Onchain] Starting', { type: params.type, target: params.target });
  ...
  try {
    switch (params.type) {
      ...
      default:
        throw new Error(`Unknown onchain type: ${params.type}`);
    }
  } catch (err) {
    ...
    return {
      ...
      data: { type: params.type, target: params.target },
    };
  }
}
