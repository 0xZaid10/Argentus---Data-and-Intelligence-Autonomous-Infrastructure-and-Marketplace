...
async function rpcCall(chain: string, method: string, params: unknown[]): Promise<unknown> {
  ...
  const res = await axios.post(chainConfig.rpc, {
    jsonrpc: '2.0',
    id: 1,
    method,
    params,
...
async function explorerCall(chain: string, params: Record<string, string>): Promise<any> {
  ...
  const res = await axios.get(`${chainConfig.explorerApi}?${query}`);
  const data = res.data as any;
...
export async function getTokenProfile(tokenAddress: string, chain: string): Promise<TokenProfile> {
  ...
  // Top holders — tokenholderlist is Pro only, skip for free tier
  // For ETH specifically there's no contract address anyway
  let topHolders: Array<{ address: string; percentage: number }> = [];
  // Free alternative: use DeFiLlama or CoinGecko for holder distribution info

  // Recent large transfers — skip for native assets (no contract address)
  let recentLargeTxs: Transaction[] = [];
  try {
    if (!tokenAddress || tokenAddress === 'ethereum' || tokenAddress === 'bitcoin') throw new Error('native asset');
